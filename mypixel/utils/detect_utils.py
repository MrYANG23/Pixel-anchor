...
def get_boxes(score, geo, score_thresh=0.9, nms_thresh=0.2):
    ...
    boxes = lanms.merge_quadrangle_n9(boxes.astype('float32'), nms_thresh)  # 返回最终的预测框

    # print('-----------------经过NMS后的boxes.shape---------------:',boxes.shape)
    return boxes
...
def detect(img, model, device,img_size):
    ...
    with torch.no_grad():
        score, geo ,attention_map= model(load_pil(img).to(device))  # 从网络出来的score值和geo值
    boxes = get_boxes(score.squeeze(0).cpu().numpy(), geo.squeeze(0).cpu().numpy())
    if boxes is None:
        return boxes

    #-----------------------------------添加部分(将改变后的所得推理图上的框还原到原图上)--------------------------
    boxes[:,[0,1,2,3,4,5,6,7]] /= img_size  # 除以所设置的图片size的的大小
    boxes[:, [0, 2, 4, 6]] *= width #将改变size后所得到的推理框还原到原图的尺寸大小
    boxes[:, [1, 3, 5, 7]] *= height#同上
    #-----------------------------------添加部分---------------------------
...
def plot_boxes(img, boxes):
    ...
    return img
def detect_dataset(model, device, test_img_path, submit_path,img_size=640):
    print('----------------进入detect_dataset-----------------')
    ...
    for i, img_file in enumerate(img_files):
        print('evaluating {} image'.format(i),'img_file:',img_file)
        boxes = detect(Image.open(img_file), model, device,img_size=img_size)
        seq = []
