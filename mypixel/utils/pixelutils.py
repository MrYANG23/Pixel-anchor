...
def get_score_geo(img, vertices, labels, scale, length):  # 制作score的标签以及geometry的标签。
    ...
    attention_map=np.zeros((int(img.height*scale),int(img.width*scale),1),np.float32)  # attention_map
    ...
    ignored_polys = []
    polys = []
    attention_polys=[] # 用于attention部分的标签制作
    ...
    for i, vertice in enumerate(vertices):
        ...
        attention_polys.append(np.around(scale*vertice.reshape((4,2))).astype(np.int32))
    ...
    cv2.fillPoly(attention_map,attention_polys,1)
    ...
    return torch.Tensor(score_map).permute(2, 0, 1), torch.Tensor(geo_map).permute(2, 0, 1), torch.Tensor(
        ignored_map).permute(2, 0, 1),torch.Tensor(attention_map).permute(2,0,1)
