...
def meshgrid(x, y,anchor_density,apl_name='', row_major=True):#获得feature上每个点的坐标。按照行先或者列先
    ...
    if apl_name in ['APL_01', 'APL_02', 'APL_04']:
        a = torch.arange(0, x) / anchor_density  # v3
        # print('-----------------------------------------a.size():',a.size())
        b = torch.arange(0, y)  # v3
        # print('-----------------------------------------b.size():',b.size())
        xx = a.repeat(y).view(-1, 1).float()

        # print('------------------------------------------xx.sieze():',xx.size())
        # print('-------------------传入meshgrid中的type(x)和type(y):', type(x), type(y))
        # print()

        yy = b.contiguous().view(-1, 1).repeat(1, x).contiguous().view(-1, 1).float()
        # print('------------------------------------------yy.size():',yy.size())
        return torch.cat([xx, yy], 1) if row_major else torch.cat([yy, xx], 1)
    if apl_name in ['APL_03', 'APL_05']:

        a = torch.arange(0,x)   #v3
        b = torch.arange(0,y) /anchor_density       #v3
        xx = a.repeat(y).view(-1, 1).float()
...
def change_box_order(boxes, order):
    ...
    elif order is 'xywh2xyxy':#(minx,miny,maxx,maxy)从已知框的中心坐标转为知道x,y同时最小和最大顶点坐标，size(N*4)
        a = boxes[:,:2]
        b = boxes[:,2:]
        new_boxes = torch.cat([a-b/2,a+b/2], 1)
        
    elif order is 'xywh2quad':#从已知框的中心坐标和宽高，转为4个顶点坐标集合 size(N*8)
...
def box_iou(box1, box2):
    # print("-------------------box_iou中box1.size()和box2.size()----------------",box1.size(),box2.size())
    # print("-------------------box_iou中type(box1)和type(box2)------------------",type(box1),type(box2))
    ...
    Args:
      ...
      box2: (tensor) gt_rect_boxes, sized [M,4]. xyxy  box2为图片上文本框4个顶点坐标信息，转化为（xmin,ymin,xmax,ymax）后的坐标。
    ...
    lt = torch.max(box1[:,None,:2], box2[:,:2].float())  # [N,M,2]
    rb = torch.min(box1[:,None,2:], box2[:,2:].float())  # [N,M,2]
...
def one_hot_embedding(labels, num_classes):#将标签转换为one_hot形式
