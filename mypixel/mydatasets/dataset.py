...
class custom_dataset(data.Dataset):
    ...
    def __getitem__(self,index ):
        ...
        #--------------------------------------------pixel部分制作标签---------------------
        score_map, geo_map, ignored_map ,attention_map= get_score_geo(img, boxes.numpy(), labels.numpy(), self.pixel_scale, self.lenght)
        #------------------anchor部分标签---------------
       ...
       #返回每张图上产生的先验anchor与之有最大iou标签框间相对于每个anchor的（w,h）间的偏移和每个anchor对应的标签。
        ...
        return anchor_img, score_map, geo_map, ignored_map,attention_map
    def __len__(self):
        return self.number

class new_custom_dataset(data.Dataset):
    def __init__(self,img_label_path,pixle_scale=0.25,img_size=640):
        super(new_custom_dataset, self).__init__()
        self.img_label_path=img_label_path

        self.pixle_scale=pixle_scale
        self.img_size=img_size
        self.img_dir=self.get_img_label_path(self.img_label_path)
        self.transform = Augmentation_traininig
    ...
    def __getitem__(self, index):
        ...
        label_path=os.path.join(img_path[:-4]+'.txt')
        # print('lable_path:',label_path)
        vertices,label=self.get_vertices(label_path) # 得到每张图上框的坐标以及对应的标签值
        ...
        anchor_img, boxes, labels = self.transform(size=self.img_size)(img, vertices,label)  # 经过pytorch中的transforms后图片通道数会变为c.h,w
        img = Image.fromarray(anchor_img.permute(1, 2, 0).numpy(), mode="RGB")  # 从tensor转为numpy再转为PIL

        # print('----------------------------self.pixel_scale:',self.pixle_scale)
        score_map, geo_map, ignored_map, attention_map = get_score_geo(img, boxes.numpy(), labels.numpy(),self.pixle_scale, self.img_size)

        return anchor_img, score_map, geo_map, ignored_map,attention_map

    def __len__(self):
        return len(self.img_dir)
    def get_img_label_path(self,img_lable_path):
        dir_list=os.listdir(img_lable_path)
        dir_list=[i[:] for i in dir_list if 'jpg' in i]
        dataset_len=len(dir_list)
        img_dir=[]
        for i in dir_list:
            img_path=os.path.join(img_lable_path,i)
            img_dir.append(img_path)

        return img_dir

    def get_vertices(self,lable_path):
...
def test1():
    img_label_path='/data/yanghan/pixel_link_train_data_xin_20191120'
    mydataset=new_custom_dataset(img_label_path)
    dataloader=torch.utils.data.DataLoader(mydataset,batch_size=1,shuffle=False,num_workers=1)
    print('len(dataloader):',len(dataloader))
    for n,(anchor_img, score_map, geo_map, ignored_map,attention_map) in enumerate(dataloader):
        print('anchor_img.size()',anchor_img.size())
        print('score_map.size()',score_map.size())
        print('geo_map.size()',geo_map.size())
        print('ignored_map()',ignored_map.size())
