...
class FocalLoss(nn.Module):
    ...
    def forward(self, loc_preds, loc_targets, cls_preds, cls_targets):
        ...
        ################################################################
        # pos_neg = cls_targets > 0  # exclude ignored anchors
        # mask = pos_neg.unsqueeze(2).expand_as(cls_preds)
        #
        # masked_cls_preds = cls_preds[mask].view(-1, self.num_classes)
        #
        #
        # #--------------------------------------修改的anchor_loss中使用的分类loss(交叉熵)
        # cls_loss = F.binary_cross_entropy(masked_cls_preds.float(), cls_targets[pos_neg].float())
        print('------------------------------anchor中的cls_loss-----------------:',cls_loss)
        print('------------------------------anchor中的loc_loss-----------------：',loc_loss)
        #-----------------------------------------------------------------------------------#

        anchor_loss = 0.2 * (loc_loss / num_pos) + (cls_loss / num_pos)
        print('--------------------------------anchor_loss----------------------:',anchor_loss)
        return anchor_loss
...
class NewFocalLoss(nn.Module):
    ...
    def forward(self, loc_preds, loc_targets, cls_preds, cls_targets):
        ...
        masked_cls_preds = cls_preds[mask].view(-1, self.num_classes)
        print('-----------------------anchor_loss中经过掩码的预测标签masked_cls_preds.size():',masked_cls_preds.size())
        print('-----------------------anchor_loss中经过掩码的真实标签cls_targets[pos_neg].size():',cls_targets[pos_neg].size())
        ...
        cls_loss = F.binary_cross_entropy(masked_cls_preds.squeeze().float(), cls_targets[pos_neg].float())
...
class OriginalFocalLoss(nn.Module):
    ...
    def forward(self, loc_preds, loc_targets, cls_preds, cls_targets):
        ...
        print('-------------------masked_cls_preds:',masked_cls_preds)
        print('-------------------cls_targets[pos_neg]:',cls_targets[pos_neg])

        cls_loss = self.focal_loss(masked_cls_preds, cls_targets[pos_neg])

        #-----------------------用BCElosss---------------

        # cls_loss=torch.nn.BCELoss(masked_cls_preds,cls_targets[pos_neg])
        #-----------------------用BCEloss
        print('-------------------------------Focal_loss中的loc_loss:',loc_loss)
        print('-------------------------------Focal_loss中的cls_loss:',cls_loss)
        anchor_loss = (loc_loss / num_pos) + (cls_loss / num_pos)
