...
class custom_dataset(data.Dataset):
    ...
    def __getitem__(self,index ):
        ...
        # transform1 = transforms.Compose([transforms.ColorJitter(0.5, 0.5, 0.5, 0.25), \
        #                                 # transforms.ToTensor(), \
       ...
       #返回每张图上产生的先验anchor与之有最大iou标签框间相对于每个anchor的（w,h）间的偏移和每个anchor对应的标签。
        ...
        return anchor_img, score_map, geo_map, ignored_map,attention_map,loc_target,cls_target
    def __len__(self):
        return self.number
#------------------------------测试mydataset--------------------------
def test():
